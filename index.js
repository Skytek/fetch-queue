  ...
  let promises = tasks.map(async (task) => {
    ...
    return fetch(queue.config.baseUrl + task.url, options)
      .then((resp) => {
        ...
        if (!resp.ok || queue.config.retryOn.indexOf(resp.status) > 1 || queue.config.retryOn.indexOf(httpErrorXX) > 1) {
...
export { fetchQueue, createQueue, checkQueue, killQueue, destroyQueue, debugQueue };
